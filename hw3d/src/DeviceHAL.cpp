...
std::shared_ptr<i_mouse> device_hal::get_mouse()
{
	return std::shared_ptr{ std::move(p_mouse_) };
}

std::shared_ptr<i_keyboard> device_hal::get_keyboard()
{
	return std::shared_ptr{ std::move(p_keyboard_) };
}

std::shared_ptr<i_graphics> device_hal::get_graphics(const HWND parent, int width, int height)
{
